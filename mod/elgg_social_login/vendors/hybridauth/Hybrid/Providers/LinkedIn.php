	...
	*/
	function getUserProfile()
	{
		try{
			...
			$response = $this->api->profile('~:(id,first-name,last-name,public-profile-url,picture-url,date-of-birth,phone-numbers,summary,location,email-address)');
		}
		...
		if( isset( $response['success'] ) && $response['success'] === TRUE ){
			...
			$this->user->profile->LinkedIn	  = @ (string) $data->{'public-profile-url'}; 	
			$this->user->profile->description = @ (string) $data->{'summary'}; 
			$this->user->profile->region      = @ (string) $data->{'location'}->{'name'};
