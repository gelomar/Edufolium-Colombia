	...
	if( isset( $_GET["provider"] ) && isset( $_GET["redirect_to_provider"] )){
		try{ 
			...
			# {{{ update user profile
				...
				// 2. Brief description
				$fulldescription = html_entity_decode( $user_profile->description, ENT_COMPAT, 'UTF-8');
				$shortdescription = substr($fulldescription, 0, 50);
				
				create_metadata( $user->guid, "briefdescription", $shortdescription, "text", $user->guid, 1 );
			...
			elseif (count($users) == 1) { 
				...
				forward("http://edufolium.com/colombia/teachers/home");
			}
			else{
